...
const DropzoneContent = ({
  ...
  if (isSuccess) {
    return (
      (<div
        className={cn('flex flex-row items-center gap-x-2 justify-center', className)}>
        ...
        return (
          (<div
            ...
            {file.type.startsWith('image/') ? (
              <div
                className="h-10 w-10 rounded border overflow-hidden shrink-0 bg-muted flex items-center justify-center">
                <img src={file.preview} alt={file.name} className="object-cover" />
              </div>
            ) : (
              <div
                className="h-10 w-10 rounded border bg-muted flex items-center justify-center">
                <File size={18} />
              </div>
            )}
